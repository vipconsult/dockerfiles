...
chmod -R 700 /bacula
chmod -R 700 /tmp/
cp /tmp/conf/bacula-dir.conf /tmp/
cp /tmp/conf/bacula-sd.conf /tmp/
cp /tmp/conf/bconsole.conf /tmp/
...
sed -i -e "s/DIR_NAME/$DIR_NAME/" /tmp/bacula-dir.conf
sed -i -e "s/BS_ADDR/$BS_ADDR/" /tmp/bacula-dir.conf
